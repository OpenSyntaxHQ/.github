...
## How to get started
1. Read `CONTRIBUTING.md` and `CODE_OF_CONDUCT.md` in this organization. They explain workflow, expectations, and how we treat each other.
2. Look through **Issues** and **Discussions** to find tasks or proposals. Issues = bugs/feature requests; Discussions = design and community conversations.
3. Pick an issue or propose a small scoped one. If it’s a proposal, start a discussion first to get buy-in.
4. Fork or branch in the relevant repo, implement changes with tests and documentation, open a pull request, and request review.

## Contribution workflow (summary)
- Open an issue or join the discussion before doing major work.
- Work on a feature in a dedicated branch: `username/short-description`.
- Keep PRs focused and small. Include tests and short docs for user-facing changes.
- Use clear commit messages (imperative, short). Example style: `feat: add X`, `fix: correct Y`.
- PRs require passing CI and at least one approving review from a maintainer or reviewer before merge.
...
- If unsure where to start, open a discussion and label it `help wanted` or `proposal`.
